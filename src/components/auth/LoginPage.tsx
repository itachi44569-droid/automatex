...
export function LoginPage() {
  ...
  // Redirect once auth state settles — covers Google popup and returning sessions
  useEffect(() => {
    if (!loading && user) {
      router.push("/client");
    }
      ...
      >
        <div className="glass-card border border-border/50 rounded-3xl p-8 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm mt-1">Sign in to your client portal</p>
          </div>
          ...
          <div className="relative mb-4">
            <Separator />
            <span className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 px-3 bg-card text-xs text-muted-foreground">or continue with email</span>
          </div>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <Link href="/register" className="text-blue-500 hover:underline font-medium">Create account</Link>
          </p>
        </div>
      </motion.div>
    </div>
